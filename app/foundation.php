...
function bx_settings_firebase_group_rows(array $rows, string $source): array
{
    $settings = [];
    $groups = [];
    foreach ($rows as $row) {
        ...
        $settingValue = bx_firebase_setting_value($settingName, (string) ($row['setting_value'] ?? ''));
...
function bx_settings_firebase_payload(string $projectKey = ''): array
{
    ...
    $systemSettings = bx_settings_firebase_group_rows($systemRows, 'builder_system_setting');
    $projectSettings = bx_settings_firebase_group_rows($projectRows, 'project_setting');
    $projectMedia = bx_settings_firebase_group_rows($mediaRows, 'project_setting_media');
...
function bx_sync_settings_to_firebase(string $projectKey = ''): array
{
    $projectId = bx_messenger_firebase_project_id();
    $serviceAccountPath = bx_messenger_firebase_service_account_path();
    if ($projectId === '') {
    ...
    $settingsDocument = bx_settings_firebase_payload($projectKey);
    $payload = json_encode([
        'project_id' => $projectId,
        'service_account_path' => $serviceAccountPath,
...
function bx_admin_write_project_user_to_firebase(array $row, string $password = ''): array
{
    ...
    $allowedFields = [
        ...
        'user_deactivated_at', 'user_locked_at', 'user_password_change_required',
    ];
    $safeRow = [];
...
function bx_sync_project_building_floor_to_firebase(): array
{
    $projectId = bx_messenger_firebase_project_id();
    $serviceAccountPath = bx_messenger_firebase_service_account_path();
    if ($projectId === '') {
    ...
    $payload = json_encode(['project_id' => $projectId, 'service_account_path' => $serviceAccountPath, 'rows' => bx_project_building_floor_firebase_rows()], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
    if (!is_string($payload)) {
