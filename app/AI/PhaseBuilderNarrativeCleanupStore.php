...
final class PhaseBuilderNarrativeCleanupStore
{
    ...
    public static function validateDatabaseApproval(string $draftKey, array $reply, array $sourceSnapshot): array
    {
        ...
        $contextKey = 'phase2-database-' . self::contextHash($draftKey);
        try {
            $row = is_string($projectRoot)
                ? (new PhaseAiContextStore())->read($contextKey, hash('sha256', rtrim(str_replace('\\', '/', $projectRoot), '/')))
                : null;
     ...
     * @return array<string, string>
     */
    public static function validatePersistedApproval(string $draftKey, array $reply, array $sourceSnapshot, array $grammarResponse): array
    {
        $context = [
    ...
    public function persist(string $draftKey, array $reply, array $sourceSnapshot, ?string $userKey = null): array
    {
        ...
        if ($db->BeginTrans() === false) {
            throw new RuntimeException('Narrative & Cleanup transaction could not start.');
        }
        ...
        try {
            ...
            if (is_array($existing)) {
                ...
                if ($alreadySaved) {
                    if ($db->CommitTrans() === false) {
                        throw new RuntimeException('Narrative & Cleanup transaction could not commit.');
                    }
            ...
            $audit = $db->GetRow(
                'SELECT action, module, record_key FROM builder_audit_log WHERE module = ? AND record_key = ? ORDER BY x_id DESC LIMIT 1',
                ['phase_builder_narrative_draft', $draftKey]
            );
            if (
                !is_array($audit)
                || (string) ($audit['action'] ?? '') !== ($existingDraftKey !== '' ? 'UPDATE' : 'CREATE')
                || (string) ($audit['module'] ?? '') !== 'phase_builder_narrative_draft'
                || (string) ($audit['record_key'] ?? '') !== $draftKey
            ) {
                throw new RuntimeException('Narrative & Cleanup audit read-back failed.');
            }
            if ($db->CommitTrans() === false) {
                throw new RuntimeException('Narrative & Cleanup transaction could not commit.');
            }
