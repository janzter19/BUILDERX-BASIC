...
Legacy v1 roadmap rows retain a compatibility display and export path. Current v2/v3 roadmap tasks can also be exported into `builder_phase_task`: stable phase/task/track identities update an earlier export without duplication, preserve the current task hierarchy in source metadata, retain existing completion state, and require transaction-local plus durable post-commit read-back before success is reported.
