...
function bx_admin_project_task_firebase_step(array $sync): array
{
    if (($sync['ok'] ?? false) === true) {
        $synced = max(1, (int) ($sync['synced'] ?? 1));
        return [
            'label' => 'Firebase',
            'status' => 'complete',
            'detail' => $synced . ' project task definition' . ($synced === 1 ? '' : 's') . ' synced to project_task.',
        ];
    }

    $message = trim((string) ($sync['message'] ?? 'Project task sync did not complete.'));
    return [
        'label' => 'Firebase',
        'status' => ($sync['skipped'] ?? false) === true ? 'blocked' : 'failed',
        'detail' => $message !== '' ? $message : 'Project task sync did not complete.',
    ];
}
...
if ($requestMethod === 'POST') {
    ...
    if (in_array($action, ['save_branch', 'set_branch_status', 'save_project', 'set_project_status', 'save_user_position', 'set_user_position_status', 'save_user', 'set_user_status', 'reset_user_password', 'save_group', 'set_group_status', 'save_role', 'set_role_status', 'set_permission_status', 'save_permission_matrix', 'save_form', 'set_form_status', 'clone_form', 'publish_form', 'unpublish_form', 'import_form_json', 'export_form_json', 'save_form_field', 'set_form_field_status', 'move_form_field', 'save_form_layout', 'set_form_layout_status', 'save_system_settings', 'set_sharingan_enabled', 'save_bed_treatment', 'set_bed_treatment_status', 'save_bed_source', 'set_bed_source_status', 'sync_bed_reference_firebase', 'update_bed_reference_sort_order', 'sync_project_building_floor', 'set_project_building_floor_status', 'update_project_building_floor_sort_order', 'resync_bed_master_list', 'resync_project_bed', 'sync_project_task_firebase', 'create_project_task', 'update_project_task', 'delete_project_task', 'update_project_task_canvas_position', 'create_project_task_stage', 'update_project_task_stage', 'delete_project_task_stage', 'create_project_task_stage_response', 'update_project_task_stage_response', 'delete_project_task_stage_response', 'update_project_task_stage_connection', 'update_project_task_stage_sort_order', 'update_project_task_stage_response_sort_order', 'apply_runtime_project_config', 'save_template_preset', 'run_template_command'], true)) {
        ...
        $syncProjectTaskDefinition = static function (string $taskKey): array {
            $taskRows = bx_project_task_firebase_rows($taskKey, 1);
            $firebaseSync = bx_sync_project_task_rows_to_firebase($taskRows);
            $firebaseStep = bx_admin_project_task_firebase_step($firebaseSync);

            return [
                'rows' => $taskRows,
                'sync' => $firebaseSync,
                'step' => $firebaseStep,
                'ok' => $firebaseStep['status'] === 'complete',
            ];
        };

        if ($action === 'sync_project_task_firebase') {
            $taskBuilderSelectedTaskKey = trim((string) ($_POST['selected_task_key'] ?? ''));
            try {
                $taskRows = bx_project_task_firebase_rows();
                $firebaseSync = bx_sync_project_task_rows_to_firebase($taskRows);
                $firebaseStep = bx_admin_project_task_firebase_step($firebaseSync);
                $firebaseOk = $firebaseStep['status'] === 'complete';
                bx_audit('SYNC', 'project_task', 'all', [
                    'row_count' => count($taskRows),
                    'firebase_ok' => $firebaseOk,
                    'firebase_collection' => 'project_task',
                ], 'Administrator synced project tasks to Firebase.');
                bx_mutation_lifecycle_flash($firebaseOk ? 'Project tasks synced.' : 'Project task sync needs attention.', $firebaseOk ? 'success' : 'error', [
                    ['label' => 'Authorization', 'status' => 'complete', 'detail' => 'Administrator authorization and CSRF checks passed.'],
                    ['label' => 'Read-back', 'status' => 'complete', 'detail' => (string) count($taskRows) . ' project task definition(s) prepared with stages and responses.'],
                    $firebaseStep,
                ], $firebaseOk ? null : (string) ($firebaseSync['message'] ?? 'Firebase sync did not complete.'));
            } catch (Throwable $error) {
                bx_mutation_lifecycle_flash('Project task sync failed.', 'error', [
                    ['label' => 'Authorization', 'status' => 'complete', 'detail' => 'Administrator authorization and CSRF checks passed.'],
                    ['label' => 'Read-back', 'status' => 'blocked', 'detail' => $error->getMessage()],
                    ['label' => 'Firebase', 'status' => 'not_started', 'detail' => 'No project task sync was completed.'],
                ], $error->getMessage());
            }
        ...
        if ($action === 'update_project_task_canvas_position') {
            try {
                ...
                $firebaseResult = $syncProjectTaskDefinition((string) ($task['task_key'] ?? ''));
                bx_admin_json_response([
                    'ok' => $firebaseResult['ok'],
                    'message' => $firebaseResult['ok'] ? 'Task canvas position saved.' : 'Task canvas position saved; Firebase sync needs attention.',
                    'task' => $task,
                    'firebase_sync' => $firebaseResult['sync'],
                ]);
            } catch (Throwable $error) {
                bx_admin_json_response([
                    'ok' => false,
        ...
        if ($action === 'update_project_task_stage_sort_order') {
            try {
                ...
                $firebaseResult = $syncProjectTaskDefinition((string) ($_POST['task_key'] ?? ''));
                bx_admin_json_response([
                    'ok' => $firebaseResult['ok'],
                    'message' => $firebaseResult['ok'] ? 'Task stage order saved.' : 'Task stage order saved; Firebase sync needs attention.',
                    'stages' => $stages,
                    'firebase_sync' => $firebaseResult['sync'],
                ]);
            } catch (Throwable $error) {
        ...
        if ($action === 'update_project_task_stage_response_sort_order') {
            try {
                ...
                bx_admin_json_response([
                    'ok' => $firebaseResult['ok'],
                    'message' => $firebaseResult['ok'] ? 'Task stage response order saved.' : 'Task stage response order saved; Firebase sync needs attention.',
                    'responses' => $responses,
                    'firebase_sync' => $firebaseResult['sync'],
                ]);
            } catch (Throwable $error) {
        ...
        if ($action === 'update_project_task_stage_connection') {
            ...
            try {
                ...
                $firebaseResult = $syncProjectTaskDefinition($taskBuilderSelectedTaskKey);
                bx_mutation_lifecycle_flash($firebaseResult['ok'] ? 'Task stage connection saved and synced.' : 'Task stage connection saved; Firebase sync needs attention.', $firebaseResult['ok'] ? 'success' : 'error', [
                    ...
                    $firebaseResult['step'],
                ], $firebaseResult['ok'] ? null : (string) ($firebaseResult['sync']['message'] ?? 'Firebase sync did not complete.'));
            } catch (Throwable $error) {
        ...
        if ($action === 'sync_project_building_floor') {
            try {
                $floorRows = bx_project_building_floor_rows();
                $firebaseSync = bx_sync_project_building_floor_to_firebase();
                $firebaseOk = ($firebaseSync['ok'] ?? false) === true;
                bx_mutation_lifecycle_flash($firebaseOk ? 'Building floors synced.' : 'Building floors sync needs attention.', $firebaseOk ? 'success' : 'error', [
                    ['label' => 'Authorization', 'status' => 'complete', 'detail' => 'Administrator authorization and CSRF checks passed.'],
                    ['label' => 'Read-back', 'status' => 'complete', 'detail' => (string) count($floorRows) . ' building-floor row(s) prepared.'],
                    ['label' => 'Sync', 'status' => $firebaseOk ? 'complete' : 'failed', 'detail' => $firebaseOk ? (string) ($firebaseSync['synced'] ?? count($floorRows)) . ' floor row(s) updated for connected users.' : (string) ($firebaseSync['message'] ?? 'Sync did not complete.')],
                ], $firebaseOk ? null : (string) ($firebaseSync['message'] ?? 'Sync did not complete.'));
            } catch (Throwable $error) {
                bx_mutation_lifecycle_flash('Building floors sync failed.', 'error', [
                    ['label' => 'Authorization', 'status' => 'complete', 'detail' => 'Administrator authorization and CSRF checks passed.'],
                    ['label' => 'Read-back', 'status' => 'blocked', 'detail' => $error->getMessage()],
                    ['label' => 'Sync', 'status' => 'not_started', 'detail' => 'No building-floor sync was completed.'],
                ], $error->getMessage());
            }
            bx_admin_redirect('floor-management');
        }

        if ($action === 'update_project_building_floor_sort_order') {
            try {
                $rows = bx_update_project_building_floor_sort_order($_POST, (string) ($currentUser['user_key'] ?? ''));
                $firebaseSync = bx_sync_project_building_floor_to_firebase();
                bx_admin_json_response([
                    'ok' => true,
                    'message' => ($firebaseSync['ok'] ?? false) === true ? 'Floor order saved and synced.' : 'Floor order saved; sync needs attention.',
                    'rows' => $rows,
                    'firebase_sync' => $firebaseSync,
                ]);
            } catch (Throwable $error) {
        ...
        if ($action === 'set_project_building_floor_status') {
            try {
                $floor = bx_set_project_building_floor_status($_POST, (string) ($currentUser['user_key'] ?? ''));
                $firebaseSync = bx_sync_project_building_floor_to_firebase();
                $firebaseOk = ($firebaseSync['ok'] ?? false) === true;
                bx_mutation_lifecycle_flash($firebaseOk ? 'Building floor status updated.' : 'Building floor status saved; sync needs attention.', $firebaseOk ? 'success' : 'error', [
                    ['label' => 'Validation', 'status' => 'complete', 'detail' => 'Building floor status accepted.'],
                    ['label' => 'Persistence', 'status' => 'complete', 'detail' => 'Building floor status is ' . (string) ($floor['floor_status'] ?? '') . '.'],
                    ['label' => 'Read-back', 'status' => 'complete', 'detail' => 'Building floor status was verified after update.'],
                    ['label' => 'Sync', 'status' => $firebaseOk ? 'complete' : 'failed', 'detail' => $firebaseOk ? (string) ($firebaseSync['synced'] ?? 'Building floor rows') . ' updated for connected users.' : (string) ($firebaseSync['message'] ?? 'Sync did not complete.')],
                ], $firebaseOk ? null : (string) ($firebaseSync['message'] ?? 'Firebase sync did not complete.'));
            } catch (Throwable $error) {
                bx_mutation_lifecycle_flash('Building floor status was not updated.', 'error', [
                    ['label' => 'Validation', 'status' => 'action_required', 'detail' => $error->getMessage()],
                    ['label' => 'Persistence', 'status' => 'not_started', 'detail' => 'No verified building floor status update was completed.'],
                ], $error->getMessage());
            }
            bx_admin_redirect('floor-management');
        }
        ...
        if ($action === 'create_project_task_stage') {
            ...
            try {
                ...
                $taskBuilderSelectedTaskKey = (string) ($stage['task_key'] ?? '');
                $firebaseResult = $syncProjectTaskDefinition($taskBuilderSelectedTaskKey);
                bx_mutation_lifecycle_flash($firebaseResult['ok'] ? 'Task stage created and synced.' : 'Task stage created; Firebase sync needs attention.', $firebaseResult['ok'] ? 'success' : 'error', [
        ...
        if ($action === 'update_project_task_stage') {
            ...
            try {
                ...
                $taskBuilderSelectedTaskKey = (string) ($stage['task_key'] ?? '');
                $firebaseResult = $syncProjectTaskDefinition($taskBuilderSelectedTaskKey);
                bx_mutation_lifecycle_flash($firebaseResult['ok'] ? 'Task stage updated and synced.' : 'Task stage updated; Firebase sync needs attention.', $firebaseResult['ok'] ? 'success' : 'error', [
                    ['label' => 'Validation', 'status' => 'complete', 'detail' => 'Stage fields accepted.'],
        ...
        if ($action === 'delete_project_task_stage') {
            ...
            try {
                ...
                $taskBuilderSelectedTaskKey = (string) ($stage['task_key'] ?? '');
                $firebaseResult = $syncProjectTaskDefinition($taskBuilderSelectedTaskKey);
                bx_mutation_lifecycle_flash($firebaseResult['ok'] ? 'Task stage deleted and synced.' : 'Task stage deleted; Firebase sync needs attention.', $firebaseResult['ok'] ? 'success' : 'error', [
        ...
        if ($action === 'create_project_task_stage_response') {
            ...
            try {
                ...
                $firebaseResult = $syncProjectTaskDefinition($taskBuilderSelectedTaskKey);
                bx_mutation_lifecycle_flash($firebaseResult['ok'] ? 'Stage response created and synced.' : 'Stage response created; Firebase sync needs attention.', $firebaseResult['ok'] ? 'success' : 'error', [
                ...
                if ($wantsJson) {
                    bx_admin_json_response([
                        'ok' => true,
                        'message' => $firebaseResult['ok'] ? 'Stage response created.' : 'Stage response created; Firebase sync needs attention.',
                        'response' => $response,
        ...
        if ($action === 'update_project_task_stage_response') {
            ...
            try {
                ...
                $taskBuilderSelectedTaskKey = (string) ($response['task_key'] ?? '');
                $firebaseResult = $syncProjectTaskDefinition($taskBuilderSelectedTaskKey);
                bx_mutation_lifecycle_flash($firebaseResult['ok'] ? 'Stage response updated and synced.' : 'Stage response updated; Firebase sync needs attention.', $firebaseResult['ok'] ? 'success' : 'error', [
                    ['label' => 'Validation', 'status' => 'complete', 'detail' => 'Response fields accepted.'],
        ...
        if ($action === 'delete_project_task_stage_response') {
            ...
            try {
                ...
                $taskBuilderSelectedTaskKey = (string) ($response['task_key'] ?? '');
                $firebaseResult = $syncProjectTaskDefinition($taskBuilderSelectedTaskKey);
                bx_mutation_lifecycle_flash($firebaseResult['ok'] ? 'Stage response deleted and synced.' : 'Stage response deleted; Firebase sync needs attention.', $firebaseResult['ok'] ? 'success' : 'error', [
                ...
                if ($wantsJson) {
                    bx_admin_json_response([
                        'ok' => true,
                        'message' => $firebaseResult['ok'] ? 'Stage response deleted.' : 'Stage response deleted; Firebase sync needs attention.',
                        'response' => $response,
                        'responses' => bx_project_task_stage_response_rows(),
                        'firebase_sync' => $firebaseResult['sync'],
                    ]);
        ...
        if ($action === 'create_project_task') {
            ...
            try {
                ...
                $taskRows = bx_project_task_firebase_rows($taskBuilderSelectedTaskKey, 1);
                $firebaseSync = bx_sync_project_task_rows_to_firebase($taskRows);
                $firebaseStep = bx_admin_project_task_firebase_step($firebaseSync);
                $firebaseOk = $firebaseStep['status'] === 'complete';
                bx_mutation_lifecycle_flash($firebaseOk ? 'Task created and synced.' : 'Task created; Firebase sync needs attention.', $firebaseOk ? 'success' : 'error', [
        ...
        if ($action === 'update_project_task') {
            ...
            try {
                ...
                $firebaseStep = bx_admin_project_task_firebase_step($firebaseSync);
                $firebaseOk = $firebaseStep['status'] === 'complete';
                bx_mutation_lifecycle_flash($firebaseOk ? 'Task updated and synced.' : 'Task updated; Firebase sync needs attention.', $firebaseOk ? 'success' : 'error', [
                    ['label' => 'Validation', 'status' => 'complete', 'detail' => 'Task fields accepted.'],
        ...
        if ($action === 'delete_project_task') {
            ...
            try {
                ...
                $firebaseSync = bx_sync_project_task_rows_to_firebase([bx_deleted_project_task_firebase_row($deletedTaskKey, $task)]);
                $firebaseStep = bx_admin_project_task_firebase_step($firebaseSync);
                $firebaseOk = $firebaseStep['status'] === 'complete';
                bx_mutation_lifecycle_flash($firebaseOk ? 'Task deleted and synced.' : 'Task deleted; Firebase sync needs attention.', $firebaseOk ? 'success' : 'error', [
        ...
        if ($action === 'save_user') {
            ...
            $firebaseProfile = [
                ...
                'user_activated_at' => $userStatus === 'ACTIVE' && ($existingUser['user_status'] ?? '') !== 'ACTIVE' ? gmdate('c') : ($existingUser['user_activated_at'] ?? ''),
                'user_deactivated_at' => $userStatus === 'INACTIVE' && ($existingUser['user_status'] ?? '') !== 'INACTIVE' ? gmdate('c') : ($existingUser['user_deactivated_at'] ?? ''),
                'user_locked_at' => $userStatus === 'LOCKED' && ($existingUser['user_status'] ?? '') !== 'LOCKED' ? gmdate('c') : ($existingUser['user_locked_at'] ?? ''),
            ];
        ...
        if ($action === 'save_system_settings') {
            ...
            foreach ($settings as $setting) {
                ...
                if ($newValue !== (string) $setting['setting_value']) {
                    ...
                    } elseif ($settingSource === 'project_setting_media') {
                        bx_db()->Execute(
                            'UPDATE project_setting_media SET setting_value = ? WHERE setting_key = ? AND project_key = ?',
                            [$newValue, $settingKey, $settingsProjectKey]
                        );
                    } else {
                        bx_db()->Execute(
                    ...
                    $auditTable = in_array($settingSource, ['project_setting', 'project_setting_media'], true) ? $settingSource : 'builder_system_setting';
                    bx_audit('UPDATE', $auditTable, $settingKey, [
                        'setting_name' => $settingName,
                        'project_key' => in_array($settingSource, ['project_setting', 'project_setting_media'], true) ? $settingsProjectKey : '',
                    ], 'Administrator updated system setting.');
                    $changed++;
                }
            }

            $firebaseSync = bx_sync_settings_to_firebase($settingsProjectKey);
            $firebaseStep = bx_admin_settings_firebase_step($firebaseSync);
            $firebaseOk = $firebaseStep['status'] === 'complete';
            bx_mutation_lifecycle_flash($firebaseOk ? 'System settings synced.' : 'System settings saved; Firebase sync needs attention.', $firebaseOk ? 'success' : 'error', [
                ['label' => 'Authorization', 'status' => 'complete', 'detail' => 'Administrator authorization and CSRF checks passed.'],
                ['label' => 'Database', 'status' => 'complete', 'detail' => $changed > 0 ? (string) $changed . ' setting value(s) updated.' : 'No database changes were needed.'],
                ['label' => 'Read-back', 'status' => 'complete', 'detail' => 'Active system and project settings were prepared for Firebase without secret settings.'],
                $firebaseStep,
