...
import { backoffSeconds, claimReady, enqueueRevision, failureOutcome, loadPendingQueue, loadTraverseCollections, projectionDecision, projectionSql, transitionQueue } from '../scripts/firebase-mysql-sync/queue-store.mjs'
...
test('TRAVERSE configuration stores one ACTIVE row per collection', async () => {
  const statements = []
  const connection = {
    async execute(sql, params) {
      statements.push({ sql, params })
      if (sql.startsWith('SELECT')) return [[{ firebase_collection: 'project_group' }]]
      return [{ affectedRows: 1 }]
    },
    release() {},
  }
  const configured = await loadTraverseCollections({ async execute(...args) { return connection.execute(...args) } }, { project_group: COLLECTIONS.project_group })
  assert.deepEqual(configured, ['project_group'])
  assert.equal(statements.length, 1)
  assert.match(statements[0].sql, /traverse_status = 'ACTIVE'/)
})

test('TRAVERSE does not auto-enable every compiled collection when the registry is empty', async () => {
  const statements = []
  const configured = await loadTraverseCollections({
    async execute(...args) {
      statements.push({ sql: args[0], params: args[1] })
      return [[]]
    },
  }, { project_user: COLLECTIONS.project_user, project_task: COLLECTIONS.project_task })
  assert.deepEqual(configured, [])
  assert.equal(statements.length, 1)
  assert.equal(statements.some(({ sql }) => sql.includes('INSERT INTO project_traverse_document')), false)
})

test('TRAVERSE registry DDL is collection-only and has no Firebase document ID column', () => {
  const source = fs.readFileSync(path.join(root, 'scripts/firebase-mysql-sync/queue-store.mjs'), 'utf8')
  const ddl = source.match(/CREATE TABLE IF NOT EXISTS project_traverse_document \(([^`]+?)\) ENGINE=/s)?.[1] || ''
  assert.match(ddl, /xId INT\(10\) NOT NULL AUTO_INCREMENT/)
  assert.match(ddl, /firebase_collection VARCHAR\(80\) NOT NULL/)
  assert.match(ddl, /traverse_status ENUM\('ACTIVE','INACTIVE'\)/)
  assert.equal(ddl.includes('firebase_document_id'), false)
})
...
test('pending queue view excludes terminal states and supports a collection filter', async () => {
  const calls = []
  const pool = { async execute(sql, parameters) {
    calls.push({ sql, parameters })
    return [[{ x_id: 3, collection_name: 'project_task', state: 'RETRY_WAIT', attempt_count: 2 }]]
  } }
  const rows = await loadPendingQueue(pool, { collection: 'project_task', limit: 25 })
  assert.equal(rows.length, 1)
  assert.equal(calls[0].parameters[0], 'project_task')
  assert.match(calls[0].sql, /state IN \('QUEUED','CLAIMED','RETRY_WAIT','ACK_PENDING'\)/)
  assert.doesNotMatch(calls[0].sql, /ACKED|SUPERSEDED|DEAD_LETTER/)
