...
export async function ensureFieldRegistry(pool, collection, document, config) {
  ...
  try {
    ...
    for (const field of contractFields) {
      const existing = known.get(field.name)
      if (existing && (existing.inferred_type === 'null' || existing.inferred_type === 'undefined' || field.type === 'timestamp')) {
        existing.inferred_type = normalizeRegistryType(field.type)
      }
    }
    ...
    // A dynamic field that was previously observed as NULL but is no longer
    // present in Firebase cannot provide a safe SQL type. Treat it as stale
    // registry metadata and omit it; otherwise one legacy NULL entry blocks
    // every document in the collection forever.
    for (const row of rows) {
      if (contractFields.some((seed) => seed.name === row.field_name)) continue
      const entry = known.get(row.field_name)
      if (entry?.inferred_type === 'null' || entry?.inferred_type === 'undefined') continue
      next.push({ ...entry })
    }
...
export async function repairParity(pool, collection, config, document = {}) {
  ...
  try {
    ...
    if (schema.columns.length === 0) {
      ...
      // Multiple queued documents can discover a new collection together.
      // The table lock serializes normal repair, while IF NOT EXISTS keeps a
      // first-run create idempotent if another worker won the metadata race.
      await connection.query(`CREATE TABLE IF NOT EXISTS ${quoteIdentifier(contract.table)} (xId INT(10) NOT NULL AUTO_INCREMENT, ${definitions.join(', ')}, PRIMARY KEY (xId), UNIQUE KEY ${quoteIdentifier(`uq_${contract.table}_${contract.key}`)} (${quoteIdentifier(contract.key)})) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`)
