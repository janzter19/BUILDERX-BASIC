...
export async function createVerifiedBackup(connection, { table, collection, repairKind, timezone, maxTableBytes, now = new Date() }) {
  ...
  try {
    if (baselineRows.length > 0) {
      ...
      sourceRowCount = await rowCount(connection, table)
      sourceChecksum = await checksum(connection, table)
      let backupAlreadyExisted = false
      try {
        await connection.query(`CREATE TABLE ${quoteIdentifier(backupTable)} LIKE ${quoteIdentifier(table)}`)
      } catch (error) {
        // A prior process can have created this deterministic backup and died
        // before writing migration history. Reconcile it by verification; do
        // not drop or overwrite a potentially valid recovery artifact.
        const isAlreadyExists = error?.code === 'ER_TABLE_EXISTS_ERROR'
          || Number(error?.errno) === 1050
          || /already exists/i.test(String(error?.message || ''))
        if (!isAlreadyExists) throw error
        backupAlreadyExisted = true
        reused = true
      }
      ...
      backupRowCount = await rowCount(connection, backupTable)
      backupChecksum = await checksum(connection, backupTable)
      if (schemaFingerprint(backupSchema) !== preFingerprint || backupRowCount !== sourceRowCount || (sourceChecksum !== null && backupChecksum !== sourceChecksum)) throw new Error(backupAlreadyExisted ? 'backup_existing_table_mismatch' : 'backup_verification_failed')
    }
  } catch (error) {
