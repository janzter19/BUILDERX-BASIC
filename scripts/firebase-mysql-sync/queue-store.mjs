...
const CONTROL_TABLES = [
  `CREATE TABLE IF NOT EXISTS project_traverse_document (xId INT(10) NOT NULL AUTO_INCREMENT, firebase_collection VARCHAR(80) NOT NULL, firebase_document_id VARCHAR(255) NOT NULL, traverse_status ENUM('ACTIVE','INACTIVE') NOT NULL DEFAULT 'ACTIVE', PRIMARY KEY (xId), UNIQUE KEY uq_project_traverse_document (firebase_collection, firebase_document_id), KEY idx_project_traverse_document_status (traverse_status, firebase_collection)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
...
export async function loadTraverseCollections(pool, contracts) {
  ...
  if (rows.length === 0) {
    for (const collection of Object.keys(contracts)) await pool.execute(`INSERT IGNORE INTO project_traverse_document (firebase_collection, traverse_status) VALUES (?, 'ACTIVE')`, [collection])
