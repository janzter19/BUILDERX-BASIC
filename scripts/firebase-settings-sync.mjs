...
async function main() {
  ...
  const documentRef = db.collection('project_setting').doc(documentKey)
  await documentRef.set(cleanObject({
    ...document,
  ...
  if (!readBack.exists || readBack.id !== documentKey) {
    throw new Error('firebase_settings_readback_failed')
  }
