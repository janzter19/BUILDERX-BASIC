...
import { claimReady, ensureControlTables, loadTraverseCollections } from './firebase-mysql-sync/queue-store.mjs'
...
export function attachPendingListeners({ db, pool, config, collections = Object.keys(COLLECTIONS), track = (promise) => promise, isStopping = () => false, discover = discoverSnapshot }) {
  const unsubscribers = []
...
export async function runService({ config = loadConfig(), mysqlModule = mysql, firebase = null } = {}) {
  ...
  unsubscribers.push(...attachPendingListeners({ db: firebase, pool, config, collections, track, isStopping: () => stopping }))
  process.once('SIGINT', stop); process.once('SIGTERM', stop)
  telemetry('started', { collections, worker_key: workerKey })
  await Promise.all([...collections.map(scannerLoop), workerLoop()])
